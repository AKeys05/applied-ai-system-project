...
from petplanify_system import Frequency, Priority, RoutineProfile, ScheduleConstraint, Scheduler, Task
        ...
        if form_submitted:
            ...
            if success:
                ...
                st.session_state._tb_generation_notice = (
                    ...
                    "Review them below before scheduling."
                )
...
if generated_tasks:
    ...
    # ── STEP 3: Confirm ───────────────────────────────────────────────────────
        ...
        st.caption("Review complete. Scroll down to see your tasks and generate a schedule.")
...
# ── CURRENT TASKS ─────────────────────────────────────────────────────────────
all_tasks_exist = bool(owner.get_all_tasks())
if generated_tasks or all_tasks_exist:
    st.divider()
    ...
    # Generate Schedule CTA — only available once the 3-step review is complete
    review_step_key = f"task_review_step_{routine_pet_name}"
    review_done = generated_tasks and st.session_state.get(review_step_key, 1) == 3
    if review_done:
        st.divider()
        included_count = len([t for t in generated_tasks if not t.skipped])
        st.success(f"✅ Routine review complete — {included_count} task(s) ready to schedule.")
        if st.button("Continue to Schedule →", key="task_continue_to_schedule", type="primary", use_container_width=True):
            st.session_state.schedule_handoff_summary = {
                "pet_name": routine_pet_name,
                "included": included_count,
                "skipped": len([t for t in generated_tasks if t.skipped]),
                "locked": len([t for t in generated_tasks if t.locked_preferred_time]),
            }
            st.session_state.workflow_phase = "scheduling"
            st.session_state.auto_generate_daily_schedule = True
            st.switch_page("pages/schedule.py")

# ── ADVANCED: MANUAL TASK ENTRY ───────────────────────────────────────────────
st.divider()
